...
def get_random_filter(propagate: Callable,
    ...
    def filter(player_times: jnp.ndarray,
        ...
        p1_skill, p2_skill = update(p1_skill, p2_skill, match_result, static_update_params, match_key)
        ...
        return player_times, player_skills
...
def get_basic_filter(propagate: Callable,
                     update: Callable) -> Callable:
    rand_filter = get_random_filter(propagate, update)
    ...
    def filter(player_times: jnp.ndarray,
               ...
               static_propagate_params: Any,
               static_update_params: Any,
               random_key: jnp.ndarray = None) -> Tuple[jnp.ndarray, jnp.ndarray]:
        return rand_filter(player_times, player_skills, match_time, match_player_indices, match_result,
                           static_propagate_params, static_update_params, random_key)
...
def filter_sweep(filter: Callable,
    ...
    def scan_body(carry,
        ...
        int_random_key, _ = random.split(int_random_key)
        ...
        new_player_times, new_player_skills = filter(player_times,
                                                     player_skills,
                                                     match_time,
        ...
        return (new_player_times, new_player_skills, int_random_key),\
               (new_player_skills[match_player_indices[0]], new_player_skills[match_player_indices[1]])

    if random_key is None:
        random_key = random.PRNGKey(0)

    _, out_skills = scan(scan_body,
