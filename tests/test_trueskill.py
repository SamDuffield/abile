...
def test_filter():
    ...
    times = jnp.zeros(2)
    skills = jnp.array([[1.0, 0.5], [0.5, 1.3]])
    new_time = 1.0

    new_times_draw, new_skills_draw, predict_probs_draw = jit(
        abile.models.trueskill.filter
    )(
        times,
        skills,
        new_time,
...
def test_smoother():
    ...
    t = 1.0
    filter_t_mu_var = jnp.array([1.0, 0.5])

    tp1 = 2.4
    smoother_tp1_mu_var = jnp.array([1.2, 0.6])

    smoother_t_mu_var, smoother_e_t_tp1 = jit(abile.models.trueskill.smoother)(
    ...
    smoother_t_mean = filter_t_mu_var[0] + kg * (
        smoother_tp1_mu_var[0] - filter_t_mu_var[0]
    )
    smoother_t_var = (
        filter_t_mu_var[1] + kg * (smoother_tp1_mu_var[1] - predict_tp1_var) * kg
    )
    ...
    assert jnp.isclose(
        smoother_e_t_tp1, smoother_t_tp1_cov + smoother_t_mean * smoother_tp1_mu_var[0]
    )


def test_gauss_hermite():
    assert jnp.isclose(
        ghq.univariate(lambda x: x, jnp.zeros(1), jnp.ones(1), 20), 0.0, atol=1e-5
    )

    assert jnp.isclose(
        ghq.univariate(lambda x: x, jnp.zeros(1) + 0.3, jnp.ones(1) * 5, 20),
        0.3,
        atol=1e-5,
    )

    assert jnp.isclose(
        ghq.univariate(lambda x: x**2, jnp.zeros(1), jnp.ones(1) * 0.5, 20),
        0.5**2,
        atol=1e-5,
    )

    assert jnp.allclose(
        ghq.univariate(lambda x: x, jnp.zeros(5), jnp.arange(1, 6), 20),
        jnp.zeros(5),
        atol=1e-5,
    )
...
def test_maximiser():
    ...
    assert logv1 > logv2
    assert jnp.isclose(jnp.exp(logd) + jnp.exp(logv1) + jnp.exp(logv2), 1.0)
    ...
    match_indices_seq = vmap(
        lambda rk: random.choice(rk, a=jnp.arange(n_players), shape=(2,), replace=False)
    )(mi_keys)
    ...
    (
        times_by_player,
        sim_skills_by_player,
    ) = abile.times_and_skills_by_match_to_by_player(
        init_times,
        init_skills,
        match_times,
        match_indices_seq,
    ...
    max_init, max_tau, max_s_eps = abile.models.trueskill.maximiser(
        times_by_player,
        smoother_skills_extra_by_player,
        match_indices_seq,
        results,
