...
def load_epl(
    ...
    for year in range(start_date.year, end_date.year):
        ...
        data.append(
            pd.read_csv(
                f"https://www.football-data.co.uk/mmz4281/{year_str}{nyear_str}/E0.csv"
            )[cols]
        )
    ...
    data_all["HomeTeamID"] = data_all["HomeTeam"].apply(
        lambda s: players_name_to_id_dict[s]
    )
    data_all["AwayTeamID"] = data_all["AwayTeam"].apply(
        lambda s: players_name_to_id_dict[s]
    )
    ...
    if goals:
        match_results = jnp.array(data_all[["FTHG", "FTAG"]], dtype=jnp.int32)
    else:
