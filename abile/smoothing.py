...
def smoother_sweep(
    ...
    def scan_body(
        carry: Tuple[jnp.ndarray, jnp.ndarray], time_ind: int
        ...
        smooth_t_skill, extra_t = smoother(
            filter_t_skill,
            time,
            smooth_tp1_skill,
            time_plus1,
    ...
    _, (smooth_skills, extra) = scan(
        scan_body,
        (filter_single_player_skills[-1], random_key),
        jnp.arange(len(filter_single_player_times) - 2, -1, -1),
    )

    smooth_skills = jnp.vstack(
        [smooth_skills[::-1], filter_single_player_skills[-1][jnp.newaxis]]
    )
...
def times_and_skills_by_match_to_by_player(
    init_times: jnp.ndarray,
    ...
    for i in range(len(match_times)):
        ...
        skills[p1_ind] = jnp.concatenate(
            [skills[p1_ind], filter_skills_1[i][jnp.newaxis]]
        )
        skills[p2_ind] = jnp.concatenate(
            [skills[p2_ind], filter_skills_2[i][jnp.newaxis]]
        )

    return times, skills


def times_and_skills_by_player_to_by_match(
    ...
    out_skills_p1 = jnp.zeros(
        (len(match_player_indices_seq),) + skills_by_player[0][0].shape
    )
...
def expectation_maximisation(
    ...
    if match_outcomes is None:
        match_outcomes = match_results

    if match_outcomes.max() > 2:
        raise ValueError("match_outcomes must be 0, 1, or 2")
    ...
    def smoother_sweep_split(t_by_player, filter_s_by_player, prop_params, rk):
        return [
            smoother_sweep(
                smoother, t_by_player[p_ind], filter_s_by_player[p_ind], prop_params, rk
            )
            for p_ind in range(len(times_by_player))
        ]
    ...
    for i in range(n_steps):
        (
            ...
            smoother_key,
            maximiser_key,
        ) = random.split(random_key, 5)

        init_times, init_skills = initiator(
            n_players, initial_params_all[i], initiate_key
        )

        filter_skills_0, filter_skills_1, filter_pred = filter_sweep_jit(
            init_player_times=init_times,
        ...
        result_probs = jnp.array(
            [filter_pred[i, k] for i, k in enumerate(match_outcomes)]
        )
        log_ml = jnp.log(result_probs).mean()
        if verbose:
            print(
                f"Step {i + 1}/{n_steps}, \t Average prediction of result: {result_probs.mean():.3f},"
                f"\t log p(y): {log_ml:.3f}"
            )
        ...
        (
            times_by_player,
            filter_skills_by_player,
        ) = times_and_skills_by_match_to_by_player(
            init_times,
            init_skills,
            match_times,
        ...
        smoother_skills_and_extras = smoother_sweep_jit(
            times_by_player,
            filter_skills_by_player,
        ...
        new_initial_params, new_propagate_params, new_update_params = maximiser(
            times_by_player,
