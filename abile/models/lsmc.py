...
def initiator(
    ...
    return jnp.zeros(num_players) + init_time, mean + jnp.sqrt(var) * random.normal(
        random_key, shape=(num_players, n_particles)
    )
...
def smooth_single_sample(
    ...
    log_samp_probs = -jnp.square(smooth_skill_tplus1_single - filter_skill_t) / (
...
def get_sum_t1_diffs_single(
    times: jnp.ndarray, smoother_skills: jnp.ndarray
) -> Tuple[int, float]:
    time_diff = times[1:] - times[:-1]
    smoother_diff2_div_time_diff = (
        jnp.square(smoother_skills[1:] - smoother_skills[:-1])
        / time_diff[..., jnp.newaxis]
    )
    bad_inds = jnp.logical_or(
        (smoother_diff2_div_time_diff < 1e-20), jnp.isnan(smoother_diff2_div_time_diff)
    )
...
def maximiser(
    ...
    init_var_num = (
        jnp.square(init_smoothing_skills - initial_params[0]) / n_particles
    ...
    maxed_tau = jnp.sqrt(
        (
            (
                ...
                + 2 * n_particles * tau2_inv_prior_beta
            )
            / (
                num_diff_terms_and_diff_sums[:, 0].sum()
                + 2 * n_particles * (tau2_inv_prior_alpha - 1)
            )
        )
    )
    ...
    if no_draw_bool:
        ...
        def negative_expected_log_obs_dens(log_epsilon: jnp.ndarray) -> float:
            def p_y_given_x(
                skill_p1: jnp.ndarray, skill_p2: jnp.ndarray, match_result: int
            ) -> float:
            ...
            return -(
                (
                    vmap(p_y_given_x)(match_skills_p1, match_skills_p2, match_results)
                ).mean()
                + (
