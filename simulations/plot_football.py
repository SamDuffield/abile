...
match_times, match_player_indices, match_results, id_to_name, name_to_id = load_international(
...
player_involved_inds = jnp.where((player_id == match_player_indices[:, 0]) + (player_id == match_player_indices[:, 1]))
player_match_indices = match_player_indices[player_involved_inds]
player_match_indices = jnp.concatenate([jnp.zeros((1, 2), dtype=int) + player_id, player_match_indices])
player_match_indices = player_match_indices[plot_times_start:plot_times_end, :]
player_match_results = jnp.concatenate([jnp.zeros(1, dtype=int)-1, match_results[player_involved_inds]])
